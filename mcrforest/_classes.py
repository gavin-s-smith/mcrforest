...
if int(np.__version__.split('.')[0]) <= 1 and int(np.__version__.split('.')[1]) < 24:
...
class BaseDecisionTree(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None, check_input=True,
        ...
        #if self.random_state == 1025838584:
        #    print('p')
    ...
    def predict_vim(self, X, permuted_vars, mcr_ordering_pre, mcr_ordering_others, mcr_ordering_post, check_input=True):
        ...
        if is_classifier(self):
            #if np.max(proba) > 1:
            #    raise Exception('Classification returned a probability greater than 1!!!!!')


            # proba2 = proba[:, :self.n_classes_].copy()
            # normalizer = proba2.sum(axis=1)[:, np.newaxis]
            # normalizer[normalizer == 0.0] = 1.0
            # proba2 /= normalizer

            #print('Predicting: {}'.format(proba))
            #return proba

            if self.n_outputs_ == 1:
                return self.classes_.take(np.argmax(proba, axis=1), axis=0)#, proba2
