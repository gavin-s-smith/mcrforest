...
model_structure = [ ('B','A'), ('C','A'), ('C','D'), ('A','Y')]
model = BayesianModel( model_structure )

# Defining individual CPDs.
#cpd_a = TabularCPD(variable='A', variable_card=3, values=[[0.25], [0.25], [0.5]])
cpd_b = TabularCPD(variable='B', variable_card=2, values=[[0.5], [0.5]])
cpd_c = TabularCPD(variable='C', variable_card=2, values=[[0.5], [0.5]])


cpd_cd = TabularCPD(variable='D', variable_card=2, values=[[1,0], [0,1]], evidence=['C'], evidence_card=[2])
...
cpd_ya = TabularCPD(variable='Y', variable_card=2, values=[[1,0], [0,1]], evidence=['A'], evidence_card=[2])

# Associating the CPDs with the network
model.add_cpds(cpd_b, cpd_c, cpd_cd, cpd_a, cpd_ya)
...
def MCR_Random_Forest(X,y):
    #params = {'bootstrap': False, 'ccp_alpha': 0.0, 'max_depth': None, 'criterion':'gini', 'max_features': 1, 'max_leaf_nodes': None, 'max_samples': None, 'mcr_tree_equivilient_tol': 0.00001, 'min_impurity_decrease': 0.00001, 'min_impurity_split': None, 'min_samples_leaf': 1, 'min_samples_split': 2, 'min_weight_fraction_leaf': 0.0, 'n_estimators': 500, 'n_jobs': None, 'oob_score': False, 'performance_equivilence': True, 'random_state': 42, 'spoof_as_sklearn': False, 'verbose': 0, 'warm_start': False}
    #rf_best_model = RandomForestClassifier(**params)
    rf_best_model = RandomForestClassifier(n_estimators=500, max_features=1, max_depth=5, min_samples_split=2, bootstrap=False)
    rf_best_model.fit(X.values, y.values.flatten())
    rf_best_model.score(X,y)
